...
- **Habit sheet** (`src/components/home/HabitSheet.tsx` on `src/components/GlassSheet.tsx`): long-press a habit → frosted sheet slides up (expo-blur + reanimated + gesture-handler, drag down to dismiss). Shows streak/best/total + last-14-days dots, the day's proof photos, and the actions: nudge, edit, skip today, pause/resume, delete. `GlassSheet` is reusable (MonthSheet uses it too); the body scrolls when it's taller than 88% of the screen, and then only the handle/scrim dismiss. All frosted surfaces go through `src/components/Frost.tsx`: real blur on iOS; on Android expo-blur (SDK 57) only blurs when handed a `blurTarget` ref to a `BlurTargetView` wrapping the content *behind* the pane, which our sheet/tab bar/toasts (over navigator scenes) can't provide — so Android gets the library's own `none` fallback with a stronger milky tint. Same look, minus the blur.
...
- Testing on the emulator: `CI=1 npx expo start --android --port <free port>` (port 8081 was already taken by something else on this PC; CI mode makes Expo non-interactive but disables hot reload, so restart Metro after each batch of edits). Sign in as dev A/B, `adb shell input tap/swipe` + `adb exec-out screencap` for a self-driven walkthrough.
- `npm install` needs `--legacy-peer-deps` on this tree (react-i18next peer range); that was already true before this branch.

## Second pass on the ideas branch (17 Sep 2026, later)

- **Home top bar**: streak pill moved out of the calendar card entirely, now lives in `HomeTopBar`, left of the mailbox icon. Calendar card's Day/Week toggle moved onto the same row as the couple avatars/name; its active state is green (`S.accent`), not black.
- **Week view**: the day-of-week header no longer floats above everything on its own — it rides along with the first section's label row (`WeekView.tsx`, `sectionLabelInline`). Selected-day circle is green with white text now, not black-on-black (was unreadable).
- **Proof photo tile** in the habit sheet: was a fixed 96px-tall strip that cropped hard; now `aspectRatio: 3/4` so it scales properly whether shown alone or side-by-side with a partner's tile.
- **Proof of work** (new feature): a habit can require a photo to complete, with the *other* household member approving it before it counts — works for personal habits too, not just shared ones.
  - Schema: `habits.require_proof`, `completions.proof_status` (`pending`/`approved`; a reject is a delete, never stored as `'rejected'`) — migration `20260917170000_proof_of_work.sql`, applied live. RLS: a household member can flip a *partner's* pending row to approved, or delete it (reject) only while pending; a `before update` trigger blocks a "validation" update from touching any column but `proof_status`, so approving can't be used to tamper with the photo.
  - `tasksStore`: `submitProof` (creates the completion + photo + `pending` status in one write — this is how a require-proof habit gets completed, not `toggleCompletion`, which now no-ops on a fresh require-proof completion), `validateProof('approve'|'reject')`. Realtime wired to `notifyProofPending` / `notifyProofApproved` / `notifyProofRejected` (toast + local notification, same pattern as nudges).
  - `AddHabitScreen`: "Require a photo" switch, single or shared.
  - `HabitSheet`: submit-proof flow on the photo tile when a require-proof habit isn't done yet; a highlighted approve/reject block when the partner's proof is waiting on me; status badges ("Pending"/"Approved ✓") on the tiles.
  - `TaskList`: a require-proof habit can't be completed by a plain tap while my part is outstanding — tapping opens the sheet instead (undoing an already-submitted one still works as a normal tap). A small camera badge appears on any habit's icon when a photo exists for that day (not just require-proof ones), turning amber when it's a proof waiting on *my* validation.
